...
export const EventCard = ({ event }: { event: any }) => {
  return (
    <div className="flex justify-start gap-0 flex-col md:flex-row w-[22rem] md:w-[60rem]">
        ...
        <CardFooter>
          {event.is_for_external_students ? <div className="w-full flex justify-start flex-col sm:flex-row items-center gap-3">
            <Link href={event.formLinkVIT} target="_blank">
              <Button className="w-[20rem] md:w-auto">Register {"(VIT Students)"}</Button>
            </Link>
            <Link href={event.formLinkExt} target="_blank">
              <Button className="w-[20rem] md:w-auto">Register{" (External Students)"}</Button>
            </Link>
          </div> : <Link href={event.formLinkVIT} target="_blank">
            <Button className="w-[20rem] md:w-auto">Regsiter {"(VIT Students)"}</Button></Link>}
        </CardFooter>
      </Card>
    </div>
  )
